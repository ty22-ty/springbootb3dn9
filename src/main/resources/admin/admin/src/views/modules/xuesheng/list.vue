<template>
  <div class="main-content">
		    ...
		    label="政治面貌">
		     <template slot-scope="scope">
                       {{scope.row.zhengzhimianmao}}
                	                	                                    ...
                	                	                                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    prop="huodejiangxiang"
                    header-align="center"
		    label="获得奖项">
		     <template slot-scope="scope">
                       {{scope.row.huodejiangxiang}}
                     </template>
                </el-table-column>
                	                	                                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    prop="jiaoshigonghao"
		    ...
		    label="教师工号">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('xuesheng','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('xuesheng','费用结算') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="feiyongjiesuanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'费用结算':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','费用结算') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="feiyongjiesuanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'费用结算':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','费用结算') && contents.tableBtnIcon == 0" type="success" size="mini" @click="feiyongjiesuanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'费用结算':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <feiyongjiesuan-cross-add-or-update v-if="feiyongjiesuanCrossAddOrUpdateFlag" :parent="this" ref="feiyongjiesuanCrossaddOrUpdate"></feiyongjiesuan-cross-add-or-update>
...
import feiyongjiesuanCrossAddOrUpdate from "../feiyongjiesuan/add-or-update";
export default {
  data() {
    return {
      ...
      addOrUpdateFlag:false,
                  feiyongjiesuanCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
            feiyongjiesuanCrossAddOrUpdate,
          },
  methods: {
    contentStyleChange() {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            feiyongjiesuanCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.feiyongjiesuanCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xuesheng');
      this.$nextTick(() => {
        this.$refs.feiyongjiesuanCrossaddOrUpdate.init(row.id,type);
      });
    },
                    init () {
                                                                                                                                                                                  this.$http({
